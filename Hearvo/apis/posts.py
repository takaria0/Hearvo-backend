...
class PostResource(Resource):
  ...
  def get(self):
    ...
    if "id" in request.args.keys():
      ...
      page = int(request.args["page"])
      posts = Post.query.filter(Post.lang_id==lang_id, Post.content.contains(search)).join(Post.vote_selects, isouter=True).join(Post.vote_mjs, isouter=True).join(Post.mj_options, isouter=True).order_by(Post.id.desc()).distinct().paginate(page, per_page=config.POSTS_PER_PAGE).items
      status_code = 200
      post_obj = posts_schema.dump(posts)
      count_vote_obj = self._count_vote(post_obj, user_info_id)
      return count_vote_obj, status_code
